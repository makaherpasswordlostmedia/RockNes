#include <stdlib.h>
#include <string.h>
#include "plugin.h" /* rb->open/read/close/filesize - no SDL here */
...
static long long read_file_to_buffer(char *file_name, uint8_t **buffer) {
    /* Rockbox plugins get open/read/close/filesize as plain function
     * names (macro-redirected to FS_PREFIX(...) in firmware/include/
     * file.h) - no rb-> prefix needed, same as Rockboy's loader.c.
     * No SDL_IOStream here. */
    ...
    *buffer = malloc(f_size);
    ...
    ssize_t input_bytes = read(fd, *buffer, f_size);
    close(fd);
...
int load_data(ROMData *rom_data, Mapper *mapper) {
    ...
    if (strncmp((char *) header, "NESM\x1A", 5) == 0 ||
        strncmp((char *) header, "NSFE", 4) == 0) {
        /* NSF/NSFe music format - not supported in this port (the
         * NSF player module was dropped, see emulator.h). Only
         * standard .nes cartridge ROMs play on this build. */
        LOG(ERROR, "NSF files are not supported on this build");
        return -1;
    }
...
void free_mapper(Mapper *mapper) {
    ...
    if (mapper->genie != NULL)
        free(mapper->genie);
    /* mapper->NSF is always NULL in this port - NSF loading path was
     * removed, see load_data() above - so no free_NSF() call needed. */
